...
import { CreateEventSchema, EventTypeSchema, type EventType } from '@xentri/ts-schema'
...
const eventsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/api/v1/events', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      return reply
        .status(500)
        .send(problemDetails(500, 'Internal Server Error', 'Failed to create event', request.id))
    }
  })
  ...
  fastify.get('/api/v1/events', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      if (query.type) {
        ...
        if (!typeResult.success) {
          return reply
            .status(400)
            .send(
              problemDetails(400, 'Bad Request', `Invalid event type: ${query.type}`, request.id)
            )
        }
      ...
      return reply
        .status(500)
        .send(problemDetails(500, 'Internal Server Error', 'Failed to list events', request.id))
    }
  })
}
