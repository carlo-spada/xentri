...
This PRD defines WHAT the product requires. For HOW to implement:
- Event schemas: docs/platform/ts-schema/prd.md
- API implementation: docs/platform/core-api/prd.md
- Shell patterns: docs/platform/shell/prd.md
- Component specs: docs/platform/ui/prd.md
-->
...
_Note: Technical implementation details belong in `architecture.md`. This section defines the product concepts that drive the architecture._
...
**Design principle:** The intelligence is in the _timing_, not in hiding information. Calm comes from trusting the system to surface what matters when it matters.
...
1. **Immutable System Context (The Soul):**
   - _Source:_ Xentri Developers (me).
...
_These are honest metrics that reflect genuine value, not vanity. If users are referring, retaining, and expanding, the product is working._
...
**The ultimate measure:** Not market share. _Identity_. When SMBs think "how do I run my business?" — Xentri is the answer.
...
**Infrastructure Module Structure (v3.0):**

| Module | Location | PRD | Epics | Notes |
| ------ | -------- | --- | ----- | ----- |
| Shell | `apps/shell` | `docs/platform/shell/prd.md` | `docs/platform/shell/epics.md` | Astro shell with React islands |
| Core API | `services/core-api` | `docs/platform/core-api/prd.md` | `docs/platform/core-api/epics.md` | Fastify + Prisma + RLS |
| TS Schema | `packages/ts-schema` | `docs/platform/ts-schema/prd.md` | `docs/platform/ts-schema/epics.md` | Shared contracts |
| UI | `packages/ui` | `docs/platform/ui/prd.md` | `docs/platform/ui/epics.md` | Design system |

**Architecture:** See `architecture.md` for ADRs and technology decisions.

**Note:** Each Infrastructure Module has its own PRD and Epics. This Constitution PRD defines system-wide requirements (PR-xxx) and integration contracts (IC-xxx) that all modules must follow.
...
| ------------- | --------------- | ------------------- | ------------------------------------------------------------------------ |
| **Strategy**  | _Orchestration_ | **War Room**        | Cross-domain reasoning                                                   |
...
- BMAD agents orchestrate the _building_ of Xentri (code, PRDs, architecture)
- Xentri copilots will orchestrate the _running_ of businesses (strategy, sales, finance)
...
The innovation is in how these tools _connect and reason together_ — not in the tools themselves.
...
If it's wrong, they adjust. But the default is _intelligent_, not generic.
...
_Permission protocol version: 1.0 — all permission checks MUST include `permission_version: "1.0"` for forward compatibility._

**Owner-defined roles:** Each organization composes primitives into roles that match _their_ business.
...
When the user opens Xentri, they're not "checking their dashboard." They're continuing _their_ story. The system acknowledges continuity:

> _"Good morning. Yesterday you sent that proposal to Acme Corp. They haven't responded yet, but based on past patterns, they typically take 3 days. Nothing to do yet — I'll flag it if it goes quiet."_
...
The Soul doesn't just configure modules — it configures _language_.
...
> _User opens Xentri on Tuesday morning._
>
...
This is not a dashboard. This is a _briefing_. The system knows the story.
...
_This section defines the "constitution" — the rules and interfaces that all sub-categories must follow. Implementation details belong in sub-category PRDs._
...
- Actor identification (user, system, or copilot)
- Envelope version and payload schema reference
- Tracing fields (correlation_id, trace_id)
- Routing fields (priority, attention, searchable)

**See:** `docs/platform/ts-schema/prd.md` for full TypeScript/Zod schema definition (FR-TSS-001).
...
_Format: Principles + indicative ranges + hard limits + phase markers. This avoids false precision while providing clear guidance._
...
_This PRD is the constitution for the Xentri platform. All sub-category PRDs inherit from and must comply with the requirements defined here._
