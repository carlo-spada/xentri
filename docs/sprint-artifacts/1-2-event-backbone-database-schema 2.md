...
- [ ] [AI-Review][High] Enforce JWT-backed org/user verification (no header trust) before setting RLS context (services/core-api/src/middleware/orgContext.ts; services/core-api/src/routes/events.ts)
- [ ] [AI-Review][Medium] Add auth-backed spoofing tests for events routes (services/core-api/src/routes/events.test.ts)
